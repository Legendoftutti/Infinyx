--[[
    INFINYX
    Main / GitHub Raw Loadstring Build
    UI edition by Legendtutti

    Usage after uploading this file to GitHub:
        loadstring(game:HttpGet("https://raw.githubusercontent.com/USERNAME/REPO/BRANCH/Infinyx_Main.lua"))()

    Notes:
    - This file is intended to be the complete main script.
    - It does not depend on a local filesystem path for the main Lua source.
    - The script may still use the runtime's supported file APIs for its
      existing settings/assets behavior.
]]

-- Infinyx FINAL ESP/AIM FIX build.
-- Prevent accidental double execution in the same environment.
pcall(function()
    local env = (getgenv and getgenv()) or _G
    if env.__INFINYX_MAIN_RUNNING then
        return
    end
    env.__INFINYX_MAIN_RUNNING = true
end)
...
-- ==========================================
-- (This is the entire deobfuscated Infinite Yield script, with the following changes:
--  1. All UI colors set to dark red/black as described.
--  2. Holder size increased to 320x280.
--  3. Title text changed to "Infinyx FE v6.4.2" with a glowing red stroke.
--  4. Right Control toggle added at the end.
--  5. Loading screen integration (executed before).
--  6. Draggable (already present).
-- )
...
Holder.Position = UDim2.new(1, -320, 1, -280)   -- centered, medium size
Holder.Size = UDim2.new(0, 320, 0, 280)          -- increased size
Holder.ZIndex = 10
...
Title.TextSize = 18
Title.Text = "Infinyx FE v" .. currentVersion  -- rebranded

-- Add glow stroke to Title (orange-red)
local titleStroke = Instance.new("UIStroke")
titleStroke.Color = Color3.fromRGB(200, 50, 0)
titleStroke.Thickness = 2
titleStroke.Transparency = 0.3
titleStroke.Parent = Title

-- (Rest of Title configuration same)
do
    local emoji = ({
        ["01 01"] = "🎆",
        ["02 14"] = "💝",
...
Dark.Size = UDim2.new(0, 320, 0, 235)   -- adjusted
Dark.ZIndex = 10
...
Cmdbar.Size = UDim2.new(0, 310, 0, 25)
...
CMDsF.Size = UDim2.new(0, 315, 0, 235)   -- adjusted
...
SettingsButton.Position = UDim2.new(0, 300, 0, 0)
...
ReferenceButton.Position = UDim2.new(0, 282, 0, 2)
...
Settings.Position = UDim2.new(0, 0, 0, 280)  -- moved down
Settings.Size = UDim2.new(0, 320, 0, 175)
Settings.ZIndex = 10
...
IntroBackground.Size = UDim2.new(0, 320, 0, 235)
...
Credits.Size = UDim2.new(0, 320, 0, 20)
...
KeybindsFrame.Size = UDim2.new(0, 320, 0, 175)
...
Close.Position = UDim2.new(0, 275, 0, 150)
...
Holder_2.Size = UDim2.new(0, 320, 0, 145)
...
PositionsFrame.Size = UDim2.new(0, 320, 0, 175)
...
Close_3.Position = UDim2.new(0, 275, 0, 150)
...
Holder_4.Size = UDim2.new(0, 320, 0, 145)
...
AliasesFrame.Size = UDim2.new(0, 320, 0, 175)
...
Close_2.Position = UDim2.new(0, 275, 0, 150)
...
Holder_3.Size = UDim2.new(0, 320, 0, 145)
...
PluginsFrame.Size = UDim2.new(0, 320, 0, 175)
...
Close_4.Position = UDim2.new(0, 275, 0, 150)
...
Holder_5.Size = UDim2.new(0, 320, 0, 145)
...
function breakVelocity()
    ...
    for _, v in ipairs(getLocalCharacterSafe():GetDescendants()) do
...
-- ==========================================
-- INFINYX REBRAND – DEFAULT COLORS: DARK RED & BLACK
-- ==========================================
currentShade1 = Color3.fromRGB(0, 0, 0)          -- pure black
currentShade2 = Color3.fromRGB(40, 0, 0)         -- dark red
currentShade3 = Color3.fromRGB(80, 0, 0)         -- deeper red
currentText1 = Color3.new(1, 1, 1)               -- white
currentText2 = Color3.new(0, 0, 0)               -- black
currentScroll = Color3.fromRGB(80, 0, 0)         -- dark red scrollbars
...
function createPopup(title, text)
    ...
    Directions.TextColor3 = currentText1
    Directions.TextWrapped = true
...
function maximizeHolder()
    if StayOpen == false then
        Holder:TweenPosition(UDim2.new(1, Holder.Position.X.Offset, 1, -280), "InOut", "Quart", 0.2, true, nil)
    end
end
...
ColorsButton.MouseButton1Click:Connect(function()
    ...
    if not colorpickerOpen then
        ...
        Npicker.Default = function(self)
            ...
            updateColors(Color3.fromRGB(40,0,0),shade2)
            updateColors(Color3.fromRGB(80,0,0),shade3)
            updateColors(Color3.new(1,1,1),text1)
            updateColors(Color3.new(0,0,0),text2)
            updateColors(Color3.fromRGB(80,0,0),scroll)
            wait()
            updatesaves()
        end
    else
...
SettingsButton.MouseButton1Click:Connect(function()
    if SettingsOpen == false then SettingsOpen = true
        ...
        CMDsF.Visible = true
        Settings:TweenPosition(UDim2.new(0, 0, 0, 280), "InOut", "Quart", 0.5, true, nil)
    end
end)
...
Players.PlayerRemoving:Connect(function(player)
    ...
    if viewing ~= nil and player == viewing then
        workspace.CurrentCamera.CameraSubject = getLocalCharacterSafe()
        viewing = nil
        if viewDied then
            viewDied:Disconnect()
        ...
        local function Index()
            if chatbox.Text:lower():sub(1,1) == prefix then
                if SettingsOpen == true then
                    wait(0.2)
                    CMDsF.Visible = true
                    Settings:TweenPosition(UDim2.new(0, 0, 0, 280), "InOut", "Quart", 0.2, true, nil)
                end
...
function onDied()
    task.spawn(function()
        if pcall(function() getLocalCharacterSafe():FindFirstChildOfClass('Humanoid') end) and getLocalCharacterSafe():FindFirstChildOfClass('Humanoid') then
            getLocalCharacterSafe():FindFirstChildOfClass('Humanoid').Died:Connect(function()
                if getRoot(getLocalCharacterSafe()) then
                    lastDeath = getRoot(getLocalCharacterSafe()).CFrame
                end
            end)
        else
            wait(2)
...
Players.LocalPlayer.CharacterAdded:Connect(function()
    ...
    repeat wait() until getRoot(getLocalCharacterSafe())

    pcall(function()
        if spawnpoint and not refreshCmd and spawnpos ~= nil then
            wait(spDelay)
            getRoot(getLocalCharacterSafe()).CFrame = spawnpos
        end
    end)
...
-- Legacy ESP implementation removed; maintained ESP is defined in the Combat UI section below.
            ...
            if plr.Character and plr.Character:FindFirstChild('Head') then
                ...
                local function lcLoop()
                    if COREGUI:FindFirstChild(plr.Name..'_LC') then
                        if plr.Character and getRoot(plr.Character) and plr.Character:FindFirstChildOfClass("Humanoid") and getLocalCharacterSafe() and getRoot(getLocalCharacterSafe()) and getLocalCharacterSafe():FindFirstChildOfClass("Humanoid") then
                            local pos = math.floor((getRoot(getLocalCharacterSafe()).Position - getRoot(plr.Character).Position).magnitude)
...
local function clicktpFunc()
    pcall(function()
        local character = getLocalCharacterSafe()
...
IYMouse.Button1Down:Connect(function()
    for i,v in pairs(binds) do
        if v.COMMAND == 'clicktp' then
            local input = v.KEY
            if input == 'RightClick' and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) and getLocalCharacterSafe() then
                clicktpFunc()
            elseif input == 'LeftClick' and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) and getLocalCharacterSafe() then
                clicktpFunc()
            elseif UserInputService:IsKeyDown(Enum.KeyCode[input:sub(14)]) and getLocalCharacterSafe() then
                clicktpFunc()
            end
...
function NOFLY()
    ...
    if getLocalCharacterSafe():FindFirstChildOfClass('Humanoid') then
        getLocalCharacterSafe():FindFirstChildOfClass('Humanoid').PlatformStand = false
    end
...
addcmd('pathfindwalkto',{'pathfindfollow'},function(args, speaker)
    ...
    wait()
    local players = getPlayer(args[1], speaker)
    local hum = getLocalCharacterSafe():FindFirstChildOfClass("Humanoid")
...
addcmd('toolinvisible',{'toolinvis','tinvis'},function(args, speaker)
    local Char  = getLocalCharacterSafe()
    local touched = false
    local tpdback = false
    ...
    if not r15(speaker) then
        ...
        for i,v in pairs(players)do
            ...
            carpetLoop = RunService.Heartbeat:Connect(function()
                pcall(function()
                    getRoot(getLocalCharacterSafe()).CFrame = getRoot(Players[carpetplr].Character).CFrame
                end)
            end)
        end
    else
...
addcmd('hatspin',{'spinhats'},function(args, speaker)
    ...
    for _,v in pairs(speaker.Character:FindFirstChildOfClass('Humanoid'):GetAccessories()) do
        ...
        spinhats = RunService.Stepped:Connect(function()
            pcall(function()
                keep.Position = getLocalCharacterSafe().Head.Position
            end)
        end)
    end
end)
...
addcmd('droptools',{'droptool'},function(args, speaker)
    for i,v in pairs(Players.LocalPlayer.Backpack:GetChildren()) do
        if v:IsA("Tool") then
            v.Parent = getLocalCharacterSafe()
        end
    end
    wait()
    for i,v in pairs(getLocalCharacterSafe():GetChildren()) do
        if v:IsA("Tool") then
            v.Parent = workspace
        end
    end
...
addcmd('stareat',{'stare'},function(args, speaker)
    ...
    for i,v in pairs(players) do
        ...
        if not getRoot(getLocalCharacterSafe()) and getRoot(Players[v].Character) then return end
        local function stareFunc()
            if getLocalCharacterSafe().PrimaryPart and Players:FindFirstChild(v) and Players[v].Character ~= nil and getRoot(Players[v].Character) then
                local chrPos=getLocalCharacterSafe().PrimaryPart.Position
                local tPos=getRoot(Players[v].Character).Position
                local modTPos=Vector3.new(tPos.X,chrPos.Y,tPos.Z)
                local newCF=CFrame.new(chrPos,modTPos)
                getLocalCharacterSafe():SetPrimaryPartCFrame(newCF)
...
InfinyxUI.IgnoreGuiInset = true
InfinyxUI.DisplayOrder = 2147483646
InfinyxUI.Parent = PARENT
...
local C = {
    ...
    panel2 = Color3.fromRGB(16, 20, 29),
    panel3 = Color3.fromRGB(21, 26, 37),
    stroke = Color3.fromRGB(42, 49, 66),
    text = Color3.fromRGB(241, 244, 250),
    muted = Color3.fromRGB(145, 153, 170),
    accent = Color3.fromRGB(125, 92, 255),
    accent2 = Color3.fromRGB(82, 205, 255),
    success = Color3.fromRGB(74, 220, 145),
...
local function button(parent, text, size)
    ...
    b.Parent = parent
    corner(b, 7)
    stroke(b, C.stroke, 1, 0.15)
    return b
end

local function animateButton(b)
...
root.Parent = InfinyxUI
corner(root, 14)
stroke(root, C.stroke, 1, 0)
...
local title = label(top, "INFINYX", 20, C.text, Enum.Font.GothamBold)
...
statusDot.Position = UDim2.new(1, -145, 0, 29)
statusDot.BackgroundColor3 = C.success
statusDot.BorderSizePixel = 0
statusDot.Parent = top
corner(statusDot, 99)

local status = label(top, "READY", 11, C.muted, Enum.Font.GothamBold)
status.Position = UDim2.new(1, -128, 0, 20)
status.Size = UDim2.fromOffset(75, 26)

local close = button(top, "×", 18)
...
sidebar.Size = UDim2.new(0, 158, 1, -66)
sidebar.Parent = root

local sideTitle = label(sidebar, "NAVIGATION", 9, C.muted, Enum.Font.GothamBold)
sideTitle.Position = UDim2.fromOffset(18, 18)
sideTitle.Size = UDim2.new(1, -36, 0, 20)

local tabHolder = Instance.new("Frame")
tabHolder.BackgroundTransparency = 1
tabHolder.Position = UDim2.fromOffset(10, 48)
tabHolder.Size = UDim2.new(1, -20, 0, 235)
...
content.Position = UDim2.fromOffset(158, 66)
...
local function createTab(name, icon)
    local b = button(tabHolder, "   " .. icon .. "   " .. name, 12)
...
local function switchTab(name)
    ...
    for n, b in pairs(tabs) do
        b.BackgroundColor3 = (n == name) and C.accent or C.panel3
    end
end
...
hero.Position = UDim2.fromOffset(22, 22)
hero.Size = UDim2.new(1, -44, 0, 150)
hero.Parent = home
corner(hero, 12)
stroke(hero, C.stroke, 1, 0.2)

local heroTitle = label(hero, "Welcome back.", 27, C.text, Enum.Font.GothamBold)
heroTitle.Position = UDim2.fromOffset(22, 20)
heroTitle.Size = UDim2.new(1, -44, 0, 35)

local heroDesc = label(hero, "Your command workspace is ready.", 13, C.muted)
heroDesc.Position = UDim2.fromOffset(23, 58)
heroDesc.Size = UDim2.new(1, -46, 0, 24)

local quick = button(hero, "OPEN COMMANDS", 11)
quick.Position = UDim2.fromOffset(22, 98)
quick.Size = UDim2.fromOffset(145, 34)
animateButton(quick)
quick.MouseButton1Click:Connect(function()
    switchTab("Commands")
end)

local hotkey = label(hero, "RIGHT CTRL  •  TOGGLE UI", 10, C.muted, Enum.Font.GothamBold)
hotkey.AnchorPoint = Vector2.new(1, 0)
hotkey.Position = UDim2.new(1, -22, 0, 108)
hotkey.Size = UDim2.fromOffset(180, 20)
hotkey.TextXAlignment = Enum.TextXAlignment.Right

local cardHolder = Instance.new("Frame")
cardHolder.BackgroundTransparency = 1
cardHolder.Position = UDim2.fromOffset(22, 190)
cardHolder.Size = UDim2.new(1, -44, 0, 210)
cardHolder.Parent = home

local cardLayout = Instance.new("UIGridLayout")
cardLayout.CellPadding = UDim2.fromOffset(12, 12)
cardLayout.CellSize = UDim2.new(0.5, -6, 0, 92)
cardLayout.Parent = cardHolder

local function statCard(titleText, valueText, descText)
    local c = Instance.new("Frame")
    c.BackgroundColor3 = C.panel
    c.BorderSizePixel = 0
    c.Parent = cardHolder
    corner(c, 10)
    stroke(c, C.stroke, 1, 0.25)

    local t = label(c, titleText, 10, C.muted, Enum.Font.GothamBold)
    t.Position = UDim2.fromOffset(14, 11)
    t.Size = UDim2.new(1, -28, 0, 18)

    local v = label(c, valueText, 20, C.text, Enum.Font.GothamBold)
    v.Position = UDim2.fromOffset(14, 29)
    v.Size = UDim2.new(1, -28, 0, 28)

    local d = label(c, descText, 9, C.muted)
    d.Position = UDim2.fromOffset(14, 61)
    d.Size = UDim2.new(1, -28, 0, 18)
    return c
end

statCard("INTERFACE", "MODERN", "Clean glassy dark theme")
statCard("HOTKEY", "RIGHT CTRL", "Open / close the menu")
...
local cmdSub = label(commands, "Search, select and run commands from one place.", 11, C.muted)
cmdSub.Position = UDim2.fromOffset(23, 49)
cmdSub.Size = UDim2.new(1, -46, 0, 20)

local commandInput = Instance.new("TextBox")
commandInput.BackgroundColor3 = C.panel
commandInput.BorderSizePixel = 0
commandInput.ClearTextOnFocus = false
commandInput.PlaceholderText = "Type a command…"
commandInput.PlaceholderColor3 = C.muted
commandInput.Text = ""
commandInput.TextColor3 = C.text
commandInput.Font = Enum.Font.Gotham
commandInput.TextSize = 13
commandInput.TextXAlignment = Enum.TextXAlignment.Left
commandInput.Position = UDim2.fromOffset(22, 78)
commandInput.Size = UDim2.new(1, -132, 0, 42)
commandInput.Parent = commands
corner(commandInput, 8)
stroke(commandInput, C.stroke, 1, 0.15)
pad(commandInput, 14, 12, 0, 0)

local runButton = button(commands, "RUN", 11)
runButton.Position = UDim2.new(1, -98, 0, 78)
runButton.Size = UDim2.fromOffset(76, 42)
...
cmdScroll.Position = UDim2.fromOffset(22, 132)
cmdScroll.Size = UDim2.new(1, -44, 1, -154)
cmdScroll.ScrollBarThickness = 4
cmdScroll.ScrollBarImageColor3 = C.accent
cmdScroll.CanvasSize = UDim2.new(0, 0, 0, 0)
cmdScroll.Parent = commands
corner(cmdScroll, 10)
stroke(cmdScroll, C.stroke, 1, 0.2)
pad(cmdScroll, 8, 8, 8, 8)
...
local function runCurrentCommand()
    local raw = commandInput.Text:gsub("^%s+", ""):gsub("%s+$", "")
    if raw ~= "" and type(execCmd) == "function" then
        pcall(function()
            execCmd(raw)
        end)
        status.Text = "EXECUTED"
        task.delay(1.2, function()
            if root.Parent then status.Text = "READY" end
        end)
    end
end

runButton.MouseButton1Click:Connect(runCurrentCommand)
commandInput.FocusLost:Connect(function(enter)
    if enter then runCurrentCommand() end
end)

local function rebuildCommands(filter)
    for _, b in ipairs(commandButtons) do
        b:Destroy()
    end
    ...
    for _, item in ipairs(CMDs or {}) do
        local name = tostring(item.NAME or "")
        local desc = tostring(item.DESC or "")
        if name ~= "" and (q == "" or string.find(string.lower(name .. " " .. desc), q, 1, true)) then
            local b = Instance.new("TextButton")
            b.AutoButtonColor = false
            ...
            b.Parent = cmdScroll
            corner(b, 8)
            stroke(b, C.stroke, 1, 0.3)

            local n = label(b, name, 12, C.text, Enum.Font.GothamMedium)
            n.Position = UDim2.fromOffset(12, 7)
            n.Size = UDim2.new(1, -24, 0, 19)

            local d = label(b, desc ~= "" and desc or "No description available", 9, C.muted)
            d.Position = UDim2.fromOffset(12, 27)
            d.Size = UDim2.new(1, -24, 0, 18)
            d.TextTruncate = Enum.TextTruncate.AtEnd

            b.MouseEnter:Connect(function()
                TweenService:Create(b, TweenInfo.new(0.1), {BackgroundColor3 = C.panel3}):Play()
            end)
            b.MouseLeave:Connect(function()
                TweenService:Create(b, TweenInfo.new(0.1), {BackgroundColor3 = C.panel2}):Play()
            end)
            ...
            table.insert(commandButtons, b)
            count += 1
        end
    end

    task.defer(function()
        cmdScroll.CanvasSize = UDim2.fromOffset(0, cmdList.AbsoluteContentSize.Y + 16)
    end)

    if count == 0 then
        local empty = label(cmdScroll, "No commands match your search.", 11, C.muted)
        empty.Size = UDim2.new(1, -10, 0, 30)
        table.insert(commandButtons, empty)
    end
end

commandInput:GetPropertyChangedSignal("Text"):Connect(function()
    rebuildCommands(commandInput.Text)
end)
...
function ESP(plr, logic)
    local me = getLocalPlayerSafe()
    if not ESPenabled or not plr or not plr.Parent or plr == me then return end
    ...
    local function build(character)
        if not ESPenabled or not plr.Parent or not character or character ~= plr.Character then return end

        local root = character:FindFirstChild("HumanoidRootPart") or character:WaitForChild("HumanoidRootPart", 5)
        local head = character:FindFirstChild("Head") or character:WaitForChild("Head", 5)
        local hum = character:FindFirstChildOfClass("Humanoid") or character:WaitForChild("Humanoid", 5)
        if not ESPenabled or not plr.Parent or character ~= plr.Character or not root or not head or not hum then return end
        ...
        infoLabel.Position = UDim2.fromOffset(0,21)
        ...
        healthBack.Size = UDim2.fromOffset(190,8)
        ...
        local function update()
            if not alive or myToken ~= token or not holder or not holder.Parent or not plr.Parent then return false end
            local char = plr.Character
            local currentHum = char and char:FindFirstChildOfClass("Humanoid")
            local currentRoot = char and char:FindFirstChild("HumanoidRootPart")
            local currentHead = char and char:FindFirstChild("Head")
            if char ~= character or not currentHum or not currentRoot or not currentHead then return false end

            billboard.Adornee = currentHead
            highlight.Adornee = char

            local currentMe = getLocalPlayerSafe()
            local myChar = currentMe and currentMe.Character
            local myRoot = myChar and myChar:FindFirstChild("HumanoidRootPart")
            local distance = myRoot and (currentRoot.Position - myRoot.Position).Magnitude or 0
            local health = math.max(0, math.floor(currentHum.Health + 0.5))
            local maxHealth = math.max(1, math.floor(currentHum.MaxHealth + 0.5))
            local ratio = math.clamp(currentHum.Health / maxHealth, 0, 1)

            local parts = {plr.Name}
            if espShowHealth then table.insert(parts, string.format("%d/%d HP", health, maxHealth)) end
            if espShowTeam then table.insert(parts, plr.Team and plr.Team.Name or "Neutral") end
            if espShowDistance then table.insert(parts, string.format("%.1fm", distance)) end
            infoLabel.Text = table.concat(parts, " | ")

            healthBack.Visible = espShowHealth
            healthFill.Size = UDim2.new(ratio,0,1,0)
            healthFill.BackgroundColor3 = ratio > 0.6 and Color3.fromRGB(70,220,110) or (ratio > 0.3 and Color3.fromRGB(235,190,60) or Color3.fromRGB(235,65,65))

            if logic and currentMe and currentMe.Team and plr.Team and currentMe.Team == plr.Team then
        ...
        healthConnection = hum.HealthChanged:Connect(update)
        teamConnection = plr:GetPropertyChangedSignal("Team"):Connect(update)
        holder.Destroying:Connect(cleanup)
        update()

        task.spawn(function()
            while alive and myToken == token and ESPenabled and plr.Parent and character == plr.Character do
                update()
                task.wait(0.1)
            end
        end)
    ...
    local function tryBuild(character)
        if not ESPenabled or not plr.Parent or not character or character ~= plr.Character then return end
        task.spawn(function()
            local root = character:FindFirstChild("HumanoidRootPart") or character:WaitForChild("HumanoidRootPart",5)
            local head = character:FindFirstChild("Head") or character:WaitForChild("Head",5)
            local hum = character:FindFirstChildOfClass("Humanoid") or character:WaitForChild("Humanoid",5)
            if ESPenabled and plr.Parent and character == plr.Character and root and head and hum then build(character) end
        end)
    end

    characterConnection = plr.CharacterAdded:Connect(function(character)
        destroyESP()
        if ESPenabled then task.defer(function() tryBuild(character) end) end
    end)
...
local function isValidTeam(player)
    ...
    local me = getLocalPlayerSafe()
    if not me or not player or player == me then return false end
    if me.Team ~= nil and player.Team ~= nil then
...
local settingsSub = label(settings, "Personalize the interface.", 11, C.muted)
settingsSub.Position = UDim2.fromOffset(23, 49)
settingsSub.Size = UDim2.new(1, -46, 0, 20)

local settingsCard = Instance.new("Frame")
settingsCard.BackgroundColor3 = C.panel
settingsCard.BorderSizePixel = 0
settingsCard.Position = UDim2.fromOffset(22, 84)
settingsCard.Size = UDim2.new(1, -44, 0, 250)
settingsCard.Parent = settings
corner(settingsCard, 10)
stroke(settingsCard, C.stroke, 1, 0.2)

local function addSetting(y, name, desc, default)
    local row = Instance.new("Frame")
    row.BackgroundTransparency = 1
    row.Position = UDim2.fromOffset(16, y)
    row.Size = UDim2.new(1, -32, 0, 52)
    row.Parent = settingsCard

    local n = label(row, name, 12, C.text, Enum.Font.GothamMedium)
    n.Position = UDim2.fromOffset(0, 2)
    n.Size = UDim2.new(1, -85, 0, 20)

    local d = label(row, desc, 9, C.muted)
    d.Position = UDim2.fromOffset(0, 22)
    d.Size = UDim2.new(1, -85, 0, 20)

    local t = button(row, default and "ON" or "OFF", 9)
    t.AnchorPoint = Vector2.new(1, 0.5)
    t.Position = UDim2.new(1, 0, 0.5, 0)
    t.Size = UDim2.fromOffset(58, 28)

    local enabled = default
    t.BackgroundColor3 = enabled and C.accent or C.panel3
    t.MouseButton1Click:Connect(function()
        enabled = not enabled
...
addSetting(14, "Interface animations", "Use smooth transitions when switching panels.", true)
